...
public class DragAndDropInventory : MonoBehaviour
{
    ...
    public bool showInv;
    public bool showQuick;
    //list of items
    public List<Item> inv = new List<Item>();
    public int slotX, slotY;
    public int quickSlotX, quickSlotY;
    public Rect inventorySize;
    public Rect quickEquipSize;
    public List<Item> quickInv = new List<Item>();
    [Header("Dragging")]
    ...
    public void AddItem(int itemID)
    {
        ...
        for (int i = 0; i < quickInv.Count; i++)
        {
            if (quickInv[i].Name == null)
            {
                quickInv[i] = ItemData.CreateItem(itemID);
                Debug.Log("Add Item: " + quickInv[i].Name);
                return;
            }
        }
    }
    ...
    */
    ...
    void QuickDrag(int windowID)
    {
        #region Nested for loop
        int i = 0;
        Event e = Event.current;
        for (int x = 0; x < quickSlotX; x++)
        {
            for (int y = 0; y < quickSlotY; y++)
            {
                Rect quickSlotLocation = new Rect(scr.x * 0 + x * (scr.x * 0.75f), scr.y * 0.3f + y * (scr.x * 0.65f), scr.x * 0.75f, scr.y * 0.65f);
                GUI.Box(quickSlotLocation, "");
                //if an item is picked up by the cursor the player can move it
                #region Pickup Item
                if (quickInv[i].Name != null && e.button == 0 && e.type == EventType.MouseDown && quickSlotLocation.Contains(e.mousePosition) && !isDragging && !Input.GetKey(KeyCode.LeftShift))
                {
                    draggedItem = quickInv[i];
                    quickInv[i] = new Item();
                    isDragging = true;
                    draggedFrom = i;
                    Debug.Log("Currently dragging your " + draggedItem.Name);
                ...
                //if it is dropped on a tile it will swap places with that item
                #region Swap Item
                if (e.button == 0 && e.type == EventType.MouseUp && quickSlotLocation.Contains(e.mousePosition) && isDragging && quickInv[i].Name != null)
                {
                    Debug.Log("Swapping your " + draggedItem.Name + "with " + quickInv[i].Name);
                    quickInv[draggedFrom] = quickInv[i];
                    quickInv[i] = draggedItem;
                    draggedItem = new Item();
                    isDragging = false;
                }
                #endregion
                ...
                if (e.button == 0 && e.type == EventType.MouseUp && quickSlotLocation.Contains(e.mousePosition) && isDragging && quickInv[i].Name == null)
                {
                    Debug.Log("Placing your " + quickInv[i].Name);
                    quickInv[i] = draggedItem;
                    draggedItem = new Item();
                    isDragging = false;
                }
                ...
                if (quickInv[i].Name != null)
                {
                    GUI.DrawTexture(quickSlotLocation, quickInv[i].Icon);
                }
                #endregion
                i++;
            }
    ...
    private void Start()
    {
        ...
        quickEquipSize = new Rect(scr.x * 7.5f, scr.y, scr.x * 6, scr.y * 1f);
        ...
        for (int i = 0; i < quickSlotX * quickSlotY; i++)
        {
            quickInv.Add(new Item());
        }
    ...
    private void Update()
    {
        ...
        if (scr.x != Screen.width / 16)
        {
            ...
            scr.y = Screen.height / 9;
            inventorySize = new Rect(scr.x, scr.y, scr.x * 6, scr.y * 4.5f);
            quickEquipSize = new Rect(scr.x, scr.y, scr.x * 6, scr.y * 4.5f);
        }
    }
    #endregion
    #region OnGUI
    private void OnGUI()
    {
        ...
        if (showQuick)
        {
            quickEquipSize = ClampToScreen(GUI.Window(2, quickEquipSize, QuickDrag, "Quick Slots"));
        }
        #endregion
        #region Drop Item
